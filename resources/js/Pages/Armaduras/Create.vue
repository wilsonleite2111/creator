...
const submit = () => form.post(route('armaduras.store'));
</script>

<template>
    <Head title="Nova Armadura" />

    <AppLayout>
        <div class="max-w-3xl mx-auto">
            <div class="mb-8">
                <h1 class="text-3xl font-cinzel font-bold text-parchment-900 uppercase tracking-widest">Nova Armadura</h1>
                <p class="font-lora italic text-parchment-800 opacity-75 mt-1">Adicione uma armadura ao arsenal.</p>
            </div>
            ...
            <form @submit.prevent="submit" class="glass-parchment rounded-xl p-8 space-y-6 border border-parchment-400 shadow-xl">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.tipo" type="text" placeholder="Ex: Leve, Média, Pesada..." class="w-full bg-parchment-100 border border-parchment-400 rounded-lg px-4 py-2 font-lora focus:outline-none focus:border-blood-700" />
                    </div>
                    <div>
                        ...
                        <p v-if="form.errors.bonus_ca" class="text-blood-700 text-sm mt-1">{{ form.errors.bonus_ca }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input v-model="form.deslocamento_9m" type="text" placeholder="Ex: 9m" class="w-full bg-parchment-100 border border-parchment-400 rounded-lg px-4 py-2 font-lora focus:outline-none focus:border-blood-700" />
                    </div>
                    <div>
                        <label class="block font-cinzel font-bold text-parchment-900 text-sm uppercase tracking-wide mb-2">Deslocamento (6m)</label>
                        <input v-model="form.deslocamento_6m" type="text" placeholder="Ex: 6m" class="w-full bg-parchment-100 border border-parchment-400 rounded-lg px-4 py-2 font-lora focus:outline-none focus:border-blood-700" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input v-model="form.preco" type="text" placeholder="Ex: 200 po" class="w-full bg-parchment-100 border border-parchment-400 rounded-lg px-4 py-2 font-lora focus:outline-none focus:border-blood-700" />
                    </div>
                </div>
