...
const form = useForm({
    nome: props.talento.nome,
    tipo: props.talento.tipo ?? '',
    versao: props.talento.versao ?? '3.5',
    pre_requisitos: props.talento.pre_requisitos ?? '',
    beneficio: props.talento.beneficio ?? '',
    descricao: props.talento.descricao ?? '',
});

const submit = () => form.put(route('talentos.update', props.talento.id));
</script>

<template>
    <Head title="Editar Talento" />

    <AppLayout>
        <div class="max-w-2xl mx-auto">
            <div class="mb-8">
                <h1 class="text-3xl font-cinzel font-bold text-parchment-900 uppercase tracking-widest">Editar Talento</h1>
                <p class="font-lora italic text-parchment-800 opacity-75 mt-1">{{ talento.nome }}</p>
            </div>
