...
@section('title', 'Forjar Nova Classe')
...
<div class="max-w-4xl mx-auto">
    <div class="mb-8 flex items-center border-b-2 border-parchment-800 pb-4">
        ...
        <div>
            <h1 class="text-4xl font-cinzel font-bold text-parchment-900 drop-shadow-sm"><i class="fa-solid fa-hammer text-blood-700 mr-3"></i>Forjar Nova Classe</h1>
            <p class="text-parchment-800 mt-2 italic font-lora">"Escreva os segredos de um novo caminho."</p>
        </div>
    </div>
        ...
        <form action="{{ route('classes.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="versao" id="versao" class="w-full pl-3 pr-10 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-blood-600 focus:border-transparent font-lora text-parchment-900 appearance-none">
                            <option value="3.5" {{ old('versao') == '3.5' ? 'selected' : '' }}>D&D 3.5</option>
                            <option value="5.0" {{ old('versao') == '5.0' ? 'selected' : '' }}>D&D 5.0</option>
                            <option value="ad&d" {{ old('versao') == 'ad&d' ? 'selected' : '' }}>AD&D</option>
                        </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="dado_vida" id="dado_vida" class="w-full pl-10 pr-10 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-blood-600 focus:border-transparent font-lora text-parchment-900 appearance-none">
                            ...
                            <option value="4" {{ old('dado_vida') == '4' ? 'selected' : '' }}>d4 (Mago, Feiticeiro)</option>
                            <option value="6" {{ old('dado_vida') == '6' ? 'selected' : '' }}>d6 (Ladino, Bardo)</option>
                            <option value="8" {{ old('dado_vida') == '8' ? 'selected' : '' }}>d8 (Clérigo, Druida, Monge)</option>
                            <option value="10" {{ old('dado_vida') == '10' ? 'selected' : '' }}>d10 (Guerreiro, Paladino, Ranger)</option>
                            <option value="12" {{ old('dado_vida') == '12' ? 'selected' : '' }}>d12 (Bárbaro)</option>
                        </select>
                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                            <i class="fa-solid fa-chevron-down text-parchment-800"></i>
                        </div>
                ...
                <div>
                    ...
                    <select name="bba_progressao" id="bba_progressao" class="w-full px-3 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-blood-600 font-lora text-parchment-900">
                        <option value="">Selecione...</option>
                        <option value="ruim" {{ old('bba_progressao') == 'ruim' ? 'selected' : '' }}>Ruim (+1/2)</option>
                        <option value="media" {{ old('bba_progressao') == 'media' ? 'selected' : '' }}>Média (+3/4)</option>
                        <option value="boa" {{ old('bba_progressao') == 'boa' ? 'selected' : '' }}>Boa (+1)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-2">
                        <input type="number" name="pontos_pericia" id="pontos_pericia" value="{{ old('pontos_pericia') }}" min="2" step="2"
                        ...
                        <span class="text-parchment-800 font-bold">+ Modificador de Inteligência</span>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="resistencia_fortitude" class="w-full px-3 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-blood-600 font-lora">
                                <option value="">Selecione...</option>
                                <option value="ruim" {{ old('resistencia_fortitude') == 'ruim' ? 'selected' : '' }}>Ruim</option>
                                <option value="boa" {{ old('resistencia_fortitude') == 'boa' ? 'selected' : '' }}>Boa</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="resistencia_reflexos" class="w-full px-3 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-blood-600 font-lora">
                                <option value="">Selecione...</option>
                                <option value="ruim" {{ old('resistencia_reflexos') == 'ruim' ? 'selected' : '' }}>Ruim</option>
                                <option value="boa" {{ old('resistencia_reflexos') == 'boa' ? 'selected' : '' }}>Boa</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="resistencia_vontade" class="w-full px-3 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-blood-600 font-lora">
                                <option value="">Selecione...</option>
                                <option value="ruim" {{ old('resistencia_vontade') == 'ruim' ? 'selected' : '' }}>Ruim</option>
                                <option value="boa" {{ old('resistencia_vontade') == 'boa' ? 'selected' : '' }}>Boa</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="md:col-span-2 border-t border-parchment-400 pt-4 mt-2">
                    ...
                    <textarea name="descricao" id="descricao" rows="5" 
                        ...
                        placeholder="Escreva sobre a origem, papel no mundo e características desta classe...">{{ old('descricao') }}</textarea>
                </div>
            </div>
            ...
            <div class="flex justify-end pt-6 border-t border-parchment-400">
                ...
                <button type="submit" class="btn-medieval bg-blood-700 hover:bg-blood-600 text-white font-cinzel font-bold py-2 px-8 rounded border border-blood-600 shadow-md inline-flex items-center" @mouseenter="playHover()" @click="playSword()">
                    <i class="fa-solid fa-fire-flame-curved mr-2"></i> Forjar Classe
                </button>
            </div>
        </form>
    </div>
