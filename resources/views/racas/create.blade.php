...
@section('title', 'Forjar Nova Raça')
...
<div class="max-w-4xl mx-auto">
    <div class="mb-8 flex items-center border-b-2 border-parchment-800 pb-4">
        ...
        <div>
            <h1 class="text-4xl font-cinzel font-bold text-parchment-900 drop-shadow-sm"><i class="fa-solid fa-hammer text-magic-600 mr-3"></i>Documentar Nova Raça</h1>
            <p class="text-parchment-800 mt-2 italic font-lora">"Registre a origem de um novo povo."</p>
        </div>
    </div>
    ...
    <div class="glass-parchment p-8 rounded-lg border border-parchment-400 shadow-lg relative">
        ...
        <form action="{{ route('racas.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="tamanho" id="tamanho" class="w-full pl-3 pr-10 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-magic-600 focus:border-transparent font-lora text-parchment-900 appearance-none">
                            <option value="Miúdo" {{ old('tamanho') == 'Miúdo' ? 'selected' : '' }}>Miúdo</option>
                            <option value="Pequeno" {{ old('tamanho') == 'Pequeno' ? 'selected' : '' }}>Pequeno</option>
                            <option value="Médio" {{ old('tamanho', 'Médio') == 'Médio' ? 'selected' : '' }}>Médio</option>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="deslocamento" id="deslocamento" value="{{ old('deslocamento', 9) }}"
                            class="w-full pl-10 pr-3 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-magic-600 focus:border-transparent font-lora text-parchment-900 placeholder-parchment-400"
                            placeholder="Ex: 9">
                    </div>
                </div>
                ...
                <div class="md:col-span-2 border-t border-parchment-400 pt-4 mt-2">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="mod_{{ $key }}" value="{{ old('mod_'.$key, 0) }}" class="w-full px-3 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-magic-600 font-lora text-parchment-900 text-center">
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="md:col-span-2 border-t border-parchment-400 pt-4 mt-2">
                    ...
                    <textarea name="descricao" id="descricao" rows="5" 
                        class="w-full px-3 py-2 bg-parchment-100 border border-parchment-400 rounded focus:outline-none focus:ring-2 focus:ring-magic-600 font-lora text-parchment-900"
                        placeholder="Escreva sobre a história deste povo, sua cultura e peculiaridades...">{{ old('descricao') }}</textarea>
                </div>
            </div>
            ...
            <div class="flex justify-end pt-6 border-t border-parchment-400">
                ...
                <button type="submit" class="btn-medieval bg-magic-600 hover:bg-magic-500 text-white font-cinzel font-bold py-2 px-8 rounded border border-magic-600 shadow-md inline-flex items-center" @mouseenter="playHover()" @click="playSword()">
                    <i class="fa-solid fa-book-sparkles mr-2"></i> Documentar Raça
                </button>
            </div>
        </form>
    </div>
